...
class IrasutoyaScraper {
  ...
  // カテゴリ別のURL一覧を取得
  getCategoryUrls() {
    // 公式サイトのラベルを広くカバー（必要に応じて追加可能）
    const labels = [
      // 人・生活・関係
      '職業','仕事','家族','恋愛','結婚','子育て','学校','勉強','友達','人物','高齢者','赤ちゃん','子供','学生',
      // 健康・医療
      '健康','病気','医療','薬','病院','歯医者','運動','フィットネス','ダイエット','睡眠','メンタルヘルス',
      // お金・ビジネス
      'お金','貯金','節約','投資','副業','ビジネス','会社','経営','プレゼン',
      // 住まい・暮らし
      '家','家具','家事','掃除','洗濯','料理','キッチン','バス','トイレ','ゴミ出し','整理整頓',
      // 食べ物・飲み物
      '食べ物','料理','野菜','果物','肉','魚','飲み物','カフェ','レストラン','おやつ',
      // 動物・自然
      '動物','ペット','犬','猫','鳥','魚','昆虫','自然','植物','花','季節','春','夏','秋','冬',
      // 行事・イベント
      '行事','正月','節分','ひな祭り','こどもの日','七夕','お盆','ハロウィン','クリスマス','誕生日','結婚式','葬式','運動会','文化祭','学園祭',
      // 旅行・乗り物
      '旅行','観光','交通','乗り物','車','電車','バス','飛行機','船','自転車',
      // テクノロジー・メディア
      'パソコン','スマホ','インターネット','アプリ','SNS','ゲーム','動画','カメラ','テレビ','ラジオ',
      // 防災・安全
      '防災','地震','火事','救急','避難','警察','消防','保険',
      // 公共・マナー
      'マナー','エチケット','ルール','標識','学校生活','PTA','ボランティア',
      // 趣味・スポーツ
      '趣味','スポーツ','サッカー','野球','テニス','バスケットボール','バレーボール','ゴルフ','ヨガ',
      // 文化・芸術
      '音楽','美術','演劇','読書','図書館','博物館','美術館',
      // 仕事実務
      '会議','メール','資料作成','テレワーク','在宅勤務','勤怠',
      // その他
      'その他'
    ];

    return labels.map(name => ({ name, url: `/search/label/${encodeURIComponent(name)}` }));
  }
