...
回答例: sexed`
  ...
  getCategoryPrompt(keyword, category, format, duration) {
    ...
    const prompts = {
      sexed: `「${keyword}」について、誰も教えてくれない本当に知りたい性の知識を紹介してください。\n\n${diversityRules}\n\n**絶対に避ける内容（教科書的でつまらない）:**\n- 一般的な正常値の説明（テストステロン300-1000ng/dLなど）\n- 当たり前の安全知識（コンドーム98%など）\n- 基本的な生理現象の説明\n- 医療機関での検査の重要性\n\n**求める内容（知られざる真実）:**\n- ${keyword}の実際の体感変化（何日目から変化するか）\n- ${keyword}後の具体的な体の変化（時間軸で）\n- 年齢による実際の差（20代vs40代の具体的な違い）\n- 栄養の実際の効果（どの程度早まるか）\n- 睡眠との関係（何時間後に深い眠りになるか）\n- メンタルへの影響（集中力や判断力の変化）\n- カロリー消費や心拍の実際の数値\n- ${keyword}の成分や機能の具体的な数値\n- ${keyword}の分泌量や頻度の実際のデータ`,
